...
export default function TopTrailer({ mediaType }: TopTrailerProps) {
  ...
  // 🔥 FIX 1: Safe data handling
  useEffect(() => {
    if (data?.results && data.results.length > 0) {
      const validVideos = data.results.filter(
        (item) => item && item.backdrop_path
      );

      if (validVideos.length > 0) {
        setVideo(validVideos[getRandomNumber(validVideos.length)]);
      }
    }
  }, [data]);

  // 🔥 FIX 2: Avoid crash if video null
  useEffect(() => {
    if (video?.id) {
      getVideoDetail({ mediaType, id: video.id });
    }
  }, [video, mediaType, getVideoDetail]);

  // 🔥 FIX 3: Safe videoKey
  const videoKey = useMemo(() => {
    if (!detail?.videos?.results?.length) return null;
    return detail.videos.results[0]?.key || null;
  }, [detail]);
    ...
    <Box sx={{ position: "relative" }}>
      <Box sx={{ pb: "40%", position: "relative" }}>
        <Box sx={{ position: "absolute", width: "100%", height: "100%" }}>
          ...
          {video && (
            <>
              {/* 🔥 FIX 4: fallback image if no video */}
              {videoKey ? (
                <iframe
                  width="100%"
                  height="100%"
